...
function initializeArchiveDb(db: any) {
  db.run(`
    ...
    CREATE TABLE players (
      pointstreak_player_id INTEGER PRIMARY KEY,
      name TEXT
    );

    CREATE TABLE teams (
      pointstreak_team_link_id INTEGER PRIMARY KEY,
      league_id INTEGER,
      name TEXT NOT NULL,
      short_name TEXT
    );
    ...
    CREATE TABLE team_aliases (
      team_pointstreak_link_id INTEGER NOT NULL,
      name TEXT NOT NULL,
      is_current INTEGER NOT NULL DEFAULT 0,
      PRIMARY KEY (team_pointstreak_link_id, name),
      FOREIGN KEY (team_pointstreak_link_id) REFERENCES teams(pointstreak_team_link_id)
    );

    CREATE TABLE team_season_ids (
      team_pointstreak_link_id INTEGER NOT NULL,
      pointstreak_team_id INTEGER NOT NULL,
      PRIMARY KEY (team_pointstreak_link_id, pointstreak_team_id),
      FOREIGN KEY (team_pointstreak_link_id) REFERENCES teams(pointstreak_team_link_id)
    );
    ...
    CREATE TABLE rosters (
      ...
      team_pointstreak_link_id INTEGER,
      team_pointstreak_id INTEGER,
      team_name TEXT,
      pointstreak_player_id INTEGER,
      pointstreak_player_season_id INTEGER,
      first_name TEXT,
      last_name TEXT,
      name TEXT,
      position TEXT,
      ...
      photo_url TEXT,
      FOREIGN KEY (team_pointstreak_link_id) REFERENCES teams(pointstreak_team_link_id),
      FOREIGN KEY (pointstreak_player_id) REFERENCES players(pointstreak_player_id)
    );

    CREATE TABLE games (
      pointstreak_game_id INTEGER PRIMARY KEY,
      league_id INTEGER,
      season_id INTEGER NOT NULL,
      scheduled_at TEXT,
      status TEXT,
      home_team_pointstreak_link_id INTEGER,
      away_team_pointstreak_link_id INTEGER,
      home_team_pointstreak_id INTEGER,
      away_team_pointstreak_id INTEGER,
      home_score INTEGER,
      away_score INTEGER,
      is_tie INTEGER,
      winner_team_pointstreak_link_id INTEGER,
      loser_team_pointstreak_link_id INTEGER,
      winner_team_pointstreak_id INTEGER,
      loser_team_pointstreak_id INTEGER,
      raw_xml_file TEXT,
      FOREIGN KEY (home_team_pointstreak_link_id) REFERENCES teams(pointstreak_team_link_id),
      FOREIGN KEY (away_team_pointstreak_link_id) REFERENCES teams(pointstreak_team_link_id),
      FOREIGN KEY (winner_team_pointstreak_link_id) REFERENCES teams(pointstreak_team_link_id),
      FOREIGN KEY (loser_team_pointstreak_link_id) REFERENCES teams(pointstreak_team_link_id)
    );

    CREATE TABLE lineups (
      pointstreak_game_id INTEGER NOT NULL,
      season_id INTEGER NOT NULL,
      team_pointstreak_link_id INTEGER,
      team_pointstreak_id INTEGER,
      ...
      position TEXT,
      order_idx INTEGER,
      FOREIGN KEY (pointstreak_game_id) REFERENCES games(pointstreak_game_id),
      FOREIGN KEY (team_pointstreak_link_id) REFERENCES teams(pointstreak_team_link_id),
      FOREIGN KEY (pointstreak_player_id) REFERENCES players(pointstreak_player_id)
    );
    ...
    CREATE INDEX idx_games_home_team ON games(home_team_pointstreak_link_id);
    CREATE INDEX idx_games_away_team ON games(away_team_pointstreak_link_id);
    CREATE INDEX idx_rosters_season ON rosters(season_id);
    CREATE INDEX idx_rosters_team ON rosters(team_pointstreak_link_id);
    CREATE INDEX idx_rosters_player ON rosters(pointstreak_player_id);
    CREATE INDEX idx_lineups_game ON lineups(pointstreak_game_id);
    CREATE INDEX idx_lineups_player ON lineups(pointstreak_player_id);
    CREATE INDEX idx_lineups_team ON lineups(team_pointstreak_link_id);
    CREATE INDEX idx_batting_game ON batting_stats(pointstreak_game_id);
    CREATE INDEX idx_batting_player ON batting_stats(pointstreak_player_id);
    CREATE INDEX idx_batting_team ON batting_stats(team_pointstreak_link_id);
    CREATE INDEX idx_pitching_game ON pitching_stats(pointstreak_game_id);
    CREATE INDEX idx_pitching_player ON pitching_stats(pointstreak_player_id);
    CREATE INDEX idx_pitching_team ON pitching_stats(team_pointstreak_link_id);
    ...
    CREATE INDEX idx_season_batting_stats_team ON season_batting_stats(team_pointstreak_link_id);
    CREATE INDEX idx_season_batting_stats_player ON season_batting_stats(pointstreak_player_id);
    ...
    CREATE INDEX idx_season_pitching_stats_team ON season_pitching_stats(team_pointstreak_link_id);
    CREATE INDEX idx_season_pitching_stats_player ON season_pitching_stats(pointstreak_player_id);
    ...
    CREATE INDEX idx_season_batting_leaders_team ON season_batting_leaders(team_pointstreak_link_id);
    CREATE INDEX idx_season_batting_leaders_player ON season_batting_leaders(pointstreak_player_id);
    ...
    CREATE INDEX idx_season_pitching_leaders_team ON season_pitching_leaders(team_pointstreak_link_id);
    CREATE INDEX idx_season_pitching_leaders_player ON season_pitching_leaders(pointstreak_player_id);
    CREATE INDEX idx_innings_game ON innings(pointstreak_game_id);
    CREATE INDEX idx_innings_team ON innings(team_pointstreak_link_id);
    CREATE INDEX idx_standings_season ON standings(season_id);
    CREATE INDEX idx_standings_team ON standings(team_pointstreak_link_id);
  `);
}
...
function createArchiveViews(db: any) {
  db.run(`
    ...
    SELECT
      ...
      seasons.season_year,
      (SELECT COUNT(DISTINCT team_pointstreak_link_id) FROM standings WHERE standings.season_id = season_ids.season_id) AS teams,
      (SELECT COUNT(*) FROM games WHERE games.season_id = season_ids.season_id) AS games,
      (
        SELECT COUNT(DISTINCT pointstreak_player_id)
        FROM rosters
        WHERE rosters.season_id = season_ids.season_id AND pointstreak_player_id IS NOT NULL
      ) AS players,
    ...
    SELECT
      standings.season_id,
      seasons.name AS season_name,
      standings.team_pointstreak_link_id,
      ...
      (
        ...
        WHERE rosters.season_id = standings.season_id
          AND rosters.team_pointstreak_link_id = standings.team_pointstreak_link_id
          AND pointstreak_player_id IS NOT NULL
      ) AS rostered_players
    FROM standings
    LEFT JOIN teams ON teams.pointstreak_team_link_id = standings.team_pointstreak_link_id
    ...
    SELECT
      ...
      (
        ...
        FROM player_games
        WHERE player_games.pointstreak_player_id = season_batting_stats.pointstreak_player_id
      ) AS games_played,
    ...
    GROUP BY season_batting_stats.pointstreak_player_id;
  `);
}
...
export async function importMergedSeedsToArchiveDb(
  ...
  const insertPlayer = db.prepare("INSERT INTO players (pointstreak_player_id, name) VALUES (?, ?)");
  const insertTeam = db.prepare(
    "INSERT INTO teams (pointstreak_team_link_id, league_id, name, short_name) VALUES (?, ?, ?, ?)",
  );
  const insertTeamAlias = db.prepare(
    "INSERT INTO team_aliases (team_pointstreak_link_id, name, is_current) VALUES (?, ?, ?)",
  );
  const insertTeamSeasonId = db.prepare(
    "INSERT INTO team_season_ids (team_pointstreak_link_id, pointstreak_team_id) VALUES (?, ?)",
  );
  const insertRoster = db.prepare(`
    INSERT INTO rosters (
      league_id, season_id, team_pointstreak_link_id, team_pointstreak_id, team_name,
      pointstreak_player_id, pointstreak_player_season_id,
  ...
  const insertGame = db.prepare(`
    INSERT INTO games (
      pointstreak_game_id, league_id, season_id, scheduled_at, status,
      home_team_pointstreak_link_id, away_team_pointstreak_link_id,
      home_team_pointstreak_id, away_team_pointstreak_id,
      home_score, away_score, is_tie,
      winner_team_pointstreak_link_id, loser_team_pointstreak_link_id,
      winner_team_pointstreak_id, loser_team_pointstreak_id,
      raw_xml_file
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const insertLineup = db.prepare(`
    INSERT INTO lineups (
      pointstreak_game_id, season_id, team_pointstreak_link_id, team_pointstreak_id,
      is_home, pointstreak_player_id, name, jersey, position, order_idx
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const insertBatting = db.prepare(`
    INSERT INTO batting_stats (
      pointstreak_game_id, season_id, team_pointstreak_link_id, team_pointstreak_id,
      is_home, pointstreak_player_id, jersey, position, ab, runs, hits, hr, rbi, bb, so, sb, avg
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const insertPitching = db.prepare(`
    INSERT INTO pitching_stats (
      pointstreak_game_id, season_id, team_pointstreak_link_id, team_pointstreak_id,
      is_home, pointstreak_player_id, jersey, pitching_order, ip, hits, runs, earned_runs, bb, so, era
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const insertSeasonBatting = db.prepare(`
    INSERT INTO season_batting_stats (
      scope, league_id, season_id, team_pointstreak_link_id, team_pointstreak_id, source_team_name,
      pointstreak_player_id, pointstreak_player_season_id, player_name, jersey,
      at_bats, runs, hits, doubles, triples, home_runs, runs_batted_in, walks,
      hit_by_pitch, strikeouts, sacrifice_flies, stolen_bases, double_plays,
  ...
  const insertSeasonPitching = db.prepare(`
    INSERT INTO season_pitching_stats (
      ...
      batters_faced, games, games_started, complete_games, complete_game_losses,
  ...
  const insertSeasonBattingLeader = db.prepare(`
    INSERT INTO season_batting_leaders (
      scope, league_id, season_id, team_pointstreak_link_id, team_pointstreak_id, leader_category, rank,
      source_team_name, pointstreak_player_id, pointstreak_player_season_id, player_name, jersey,
      at_bats, runs, hits, doubles, triples, home_runs, runs_batted_in, walks,
      hit_by_pitch, strikeouts, sacrifice_flies, stolen_bases, double_plays,
      on_base_percentage, slugging_percentage, batting_average
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  const insertStanding = db.prepare(`
    INSERT INTO standings (
      league_id, season_id, team_pointstreak_link_id, team_pointstreak_id, name, games_played, wins, losses, ties, pct
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  try {
    ...
    for (const row of playerRows) {
      insertPlayer.run([
        requiredInteger(row.pointstreak_player_id, "pointstreak_player_id", "players"),
        textOrNull(row.name),
      ]);
    ...
    for (const row of teamRows) {
      const teamLinkId = requiredInteger(row.pointstreak_team_link_id, "pointstreak_team_link_id", "teams");
      ...
      for (const seasonTeamId of integerArray(row.season_team_pointstreak_ids)) {
    ...
    for (const row of rosterRows) {
      ...
      insertRoster.run([
        ...
        integerOrNull(row.team_pointstreak_link_id),
        integerOrNull(row.team_pointstreak_id),
        textOrNull(row.team_name),
        integerOrNull(row.pointstreak_player_id),
        integerOrNull(row.pointstreak_player_season_id),
    ...
    for (const row of gameRows) {
      ...
      insertGame.run([
        requiredInteger(row.pointstreak_game_id, "pointstreak_game_id", "games"),
        integerOrNull(row.league_id),
        requiredInteger(row.season_id, "season_id", "games"),
        textOrNull(row.scheduled_at),
        textOrNull(row.status),
        integerOrNull(row.home_team_pointstreak_link_id),
        integerOrNull(row.away_team_pointstreak_link_id),
        integerOrNull(row.home_team_pointstreak_id),
        integerOrNull(row.away_team_pointstreak_id),
        integerOrNull(row.home_score),
        integerOrNull(row.away_score),
        booleanToInteger(row.is_tie),
        integerOrNull(row.winner_team_pointstreak_link_id),
        integerOrNull(row.loser_team_pointstreak_link_id),
        integerOrNull(row.winner_team_pointstreak_id),
        integerOrNull(row.loser_team_pointstreak_id),
    ...
    for (const row of lineupRows) {
      ...
      insertLineup.run([
        requiredInteger(row.pointstreak_game_id, "pointstreak_game_id", "lineups"),
    ...
    for (const row of battingRows) {
      ...
      insertBatting.run([
        requiredInteger(row.pointstreak_game_id, "pointstreak_game_id", "batting_stats"),
    ...
    for (const row of pitchingRows) {
      ...
      insertPitching.run([
        requiredInteger(row.pointstreak_game_id, "pointstreak_game_id", "pitching_stats"),
    ...
    for (const row of inningRows) {
      ...
      insertInning.run([
        requiredInteger(row.pointstreak_game_id, "pointstreak_game_id", "innings"),
